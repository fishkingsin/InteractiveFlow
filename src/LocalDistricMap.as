...
/*function switchLanguage(language:String , file:String):void
		...
		function btnPressed(e:MouseEvent):void
		{
			...
			switch (e.target.name)
			{
				case "home" :
					...
					break;
				default :
					if (! mediaPlaying || STATE == PLAYING_SCREEN_SAVER )
					{
						if (isVidPause && e.target.name!="p_1_btn_6_1")
						{
							mediaPlaying = true;
							isVidPause = false;
							isWaitingFinish = true;
							waitingSprite = e.target as customButton;
							flvplayer._player.play();
		...
		{
			Logger.debug("processButtonAction ");
